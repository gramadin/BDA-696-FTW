...
drop table if exists rolling_090;
create table rolling_090 as	
select
	tbc1.team_id
	...
	, ifnull(((SUM(tbc2.stolenBase2B)+SUM(tbc2.stolenBase3B)+SUM(tbc2.stolenBaseHome))/nullif((SUM(tbc2.caughtStealing2B)+SUM(tbc2.caughtStealing3B)+SUM(tbc2.caughtStealingHome)+SUM(tbc2.stolenBase2B)+SUM(tbc2.stolenBase3B)+SUM(tbc2.stolenBaseHome)),0)),0) as SB_pct
	, SUM(tbc2.toBase) AS toBase
	, ifnull(SUM(tbc2.atBat)/nullif(SUM(tbc2.Home_Run),0),avg(tbc2.atBat)) AS AB_HR
	, SUM(tbc2.Hit)/nullif(SUM(tbc2.atBat),0) as Bat_Avg
	, SUM(tbc2.Batter_Interference) AS Batter_Interference
	, IFNULL(SUM(tbc2.Walk)/nullif(SUM(tbc2.Strikeout),0),avg(tbc2.Walk)) AS BB_K
	...
	, ifnull(SUM(tbc2.Strikeout)/nullif(SUM(tbc2.Walk),0),avg(tbc2.Strikeout)) as K_BB
	...
	, ifnull(SUM(tbc2.plateApperance)/nullif(SUM(tbc2.Strikeout),0),avg(tbc2.plateApperance)) AS PA_SO
	...
	, (1/(1+power(ifnull(nullif(SUM(tbc2.opponent_finalScore)/nullif(SUM(tbc2.finalScore),0),0),0),1.83))) as pathag_v1
	, (1/(1+power(ifnull(nullif(SUM(tbc2.opponent_finalScore)/nullif(SUM(tbc2.finalScore),0),0),0),((1.50*(log(((SUM(tbc2.finalScore)+SUM(tbc2.opponent_finalScore))/(count(tbc1.game_id)/2)))))+.045)))) as pathag_v2
	, (1/(1+power(ifnull(nullif(SUM(tbc2.opponent_finalScore)/nullif(SUM(tbc2.finalScore),0),0),0),power(((SUM(tbc2.finalScore)+SUM(tbc2.opponent_finalScore))/(count(tbc1.game_id)/2)),0.287)))) as pathag_v3
	, tc.Net_Jet_Lag
	...
	JOIN game g2 ON g2.game_id = tbc2.game_id AND
		...
		g2.local_date >= DATE_ADD(gl.local_date, INTERVAL -90 DAY)
	GROUP BY tbc1.team_id, tbc1.game_id , gl.local_date
	ORDER BY gl.local_date, tbc1.team_id;
# Establish Index
CREATE UNIQUE INDEX team_game ON rolling_090 (team_id, game_id);