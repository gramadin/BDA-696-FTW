...
echo exporting
docker exec -it db-container bash -c 'mysql -h localhost -u root -ppass --database bbdb --batch -e "select batter,(sum(Hits)/sum(atbats)) as batavg from rolling_100 group by batter order by batter;"'  > report.csv
