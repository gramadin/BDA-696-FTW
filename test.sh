...
echo exporting
docker exec -i db-container mysql docker exec -it db-container bash -c 'mysqld -h . -u root -ppass --database bbdb --batch -e "select batter,(sum(Hits)/sum(atbats)) as batavg from rolling_100 group by batter order by batter;"'   > sed 's/\t/","/g;s/^/"/;s/$/"/;s/\n//' > report.csv
echo Did it work?
